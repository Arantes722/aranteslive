...
async function twitchFetch(endpoint: string, token: string) {
  ...
  if (!response.ok) {
    ...
    console.error("TWITCH API ERROR:", response.status, error);
...
export async function getTwitchUser(
  ...
  const data = await twitchFetch(
    `/users?login=${login.toLowerCase()}`,

    token,
  );

  console.log("TWITCH USER RESPONSE:", data);
